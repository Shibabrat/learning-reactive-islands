...
# -*- coding: utf-8 -*-
"""
Generate training data with escape time and LD values of the initial conditions on the SOS
"""
import numpy as np
...
def generate_training_data(total_energy, end_time):
...
#     end_time = 30
    ...
    ## Obtain the boundary of the energy surface intersection
    ...
    # px_at_yzero = lambda x: np.sqrt(2*params[0]*(total_energy - 0.5*params[2]**2*x**2))

    # xGrid = np.linspace(xMin + 1e-10, xMax - 1e-10, 401, endpoint = True)
    # pxGrid = px_at_yzero(xGrid)
    # pxGrid[0] = 0
    # pxGrid[-1] = 0
    ...
    # xMesh, pxMesh = np.meshgrid(np.linspace(xMin, xMax, xRes), \
    #                            np.linspace(xMin, xMax, pxRes))
    ...
    escape_time = np.empty((xRes*pxRes,1))
    LD_val = np.empty((xRes*pxRes,1))
    escape_label[:] = np.NaN
    escape_time[:] = np.NAN
    LD_val[:] = np.NAN
    ...
    for i in range(xRes*pxRes):
        ...
        if ~np.isnan(pyMesh[i,0]):
            init_cond = [xMesh[i,0], yMesh[i,0], pxMesh[i,0], pyMesh[i,0], 0]

            sol = solve_ivp(HH2dof.vector_field_ld, [0, end_time], init_cond, \
                            args = params, \
    ...
    #            print(np.size(sol.t_events[3]), sol.t_events[3])
                ...
                if np.size(sol.t_events[0]) == 1 and np.size(sol.t_events[3]) == 1:
                    escape_label[i,0] = 1 
                    escape_time[i,0] = sol.t_events[0]
                    LD_val[i,0] = sol.y_events[0][0,-1]
    ...
    #                print(np.size(sol.t_events[0]),np.size(sol.t_events[3]))
                    ...
                    escape_label[i,0] = 2
                    escape_time[i,0] = sol.t_events[1]
                    LD_val[i,0] = sol.y_events[1][0,-1]
    ...
    #                print(np.size(sol.t_events[1]),np.size(sol.t_events[3]))
                    ...
                    escape_label[i,0] = 3
                    escape_time[i,0] = sol.t_events[2]
                    LD_val[i,0] = sol.y_events[2][0,-1]
    ...
    data = np.zeros((xRes*pxRes,7))
    data[:,0] = np.squeeze(xMesh)
    data[:,1] = np.squeeze(yMesh)
    data[:,2] = np.squeeze(pxMesh)
    data[:,3] = np.squeeze(pyMesh)
    data[:,4] = np.squeeze(escape_time)
    data[:,5] = np.squeeze(LD_val)
    data[:,6] = np.squeeze(escape_label)
