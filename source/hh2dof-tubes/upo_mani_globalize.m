function [xW,x0W] = upo_mani_globalize(x0po,T,frac,stbl,dir,del,tmfd,n_mfd_traj,par) 

% [xW,x0W] = upo_mani_globalize(x0po,T,frac,stbl,dir,del,param,tmfd,n_mfd_traj)
%
...
%     disp(n_mfd_traj)
    [xx,tt,phi_T,PHI] = state_transition_matrix(x0po,T,OPTIONS,par,n_mfd_traj);
...
%           tb,tf,param,OPTIONS,eventSwitch,numTimesteps);
        [x,t,te,xe,ie] = get_traj_sos(x0W_all(mfrac,1:N)', ...
...
%         elseif eqNum == 3 || eqNum == 4
%             plot3(x(:,1), x(:,2), x(:,4), ['-',colormfd]);
%             zlabel('$p_y$','interpreter','latex','fontsize',20);
%         end
        ...
        numPts = numPts + 1;
        numInterTraj = [numInterTraj; length(te)];
...
%     tubeXU1 = permute(xeU1,[3 2 1]);
    ...
    if stbl == -1,
        ...
        save(['mfd_stable_branch',num2str(dir),'_eqPt',num2str(eqNum), ...
            '_DelE',num2str(deltaE),'_t',num2str(tmfd),'.txt'], ...
            'dum', '-ASCII', '-double');
        ...
        save(['numCrossings_stable_branch',num2str(dir),'_eqPt',num2str(eqNum), ...
            '_hh2dof_deltaE',num2str(deltaE),'_t',num2str(tmfd),'.txt'], ...
            'numInterTraj', '-ASCII');
    ...
    elseif stbl == +1, 
        ...
        save(['numCrossings_unstable_branch',num2str(dir),'_eqPt',num2str(eqNum), ...
            '_hh2dof_deltaE',num2str(deltaE),'_t',num2str(tmfd),'.txt'], ...
            'numInterTraj', '-ASCII');
